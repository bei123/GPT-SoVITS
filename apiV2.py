...
class ModelManager:
    def __init__(self):
        self.models = {}
        self.loaded_to_device = {}

    def load_model(self, model_name, is_half):
        if model_name not in self.models:
            model = self._load_model_from_disk(model_name, is_half)
            self.models[model_name] = model
            self.loaded_to_device[model_name] = False
        return self.models[model_name]

    def _load_model_from_disk(self, model_name, is_half):

        model_path = f"api_Model/{model_name}/{model_name}.pth"
    ...
    def get_model(self, model_name, device):
        if model_name in self.models:
            if not self.loaded_to_device[model_name]:
                # 如果模型尚未加载到设备，则进行加载
                self.models[model_name] = self.models[model_name].to(device)
                self.loaded_to_device[model_name] = True
            return self.models[model_name]
        else:
            return None
...
model_cache = {"sovits": {}, "gpt": {}}
model_manager = ModelManager()


def load_weights_to_device(model, state_dict, device, is_half):
...
def change_sovits_weights(model_name, device, is_half=False):
    global vq_model, hps

    # 检查缓存中是否存在模型
    if model_name in model_cache["sovits"]:
        vq_model, hps = model_cache["sovits"][model_name]
        logger.info(f"从缓存中加载 SoVITS 模型: {model_name}")
        return
    ...
    try:
        ...
        logger.error(f"加载 SoVITS 模型权重失败: {e}")
        return
    ...
    vq_model = load_weights_to_device(vq_model, dict_s2["weight"], device, is_half)
    model_cache["sovits"][model_name] = (vq_model, hps)  # 缓存模型
    logger.info(f"已加载并缓存 SoVITS 模型: {model_name}")


def change_gpt_weights(model_name, device, is_half=False):

    global hz, max_sec, t2s_model, config
    hz = 50

    # 检查缓存中是否存在模型
    if model_name in model_cache["gpt"]:
        t2s_model, config = model_cache["gpt"][model_name]
        logger.info(f"从缓存中加载 GPT 模型: {model_name}")
        return
    ...
    try:
        ...
        logger.error(f"加载 GPT 模型权重失败: {e}")
        return

    config = dict_s1["config"]
    max_sec = config["data"]["max_sec"]
    t2s_model = Text2SemanticLightningModule(config, "****", is_train=False)
    t2s_model = load_weights_to_device(t2s_model, dict_s1["weight"], device, is_half)
    total_params = sum(p.nelement() for p in t2s_model.parameters())
    model_cache["gpt"][model_name] = (t2s_model, config)  # 缓存模型
    logger.info(f"已加载并缓存 GPT 模型: {model_name}")
...
def handle(text, model_name):

    # Paths and prompt configurations
    refer_wav_path = (
        "/root/GPT-SoVITS/api_Model/" + model_name + "/" + model_name + ".wav"
    )
    prompt_text = g_config.modelToPromptText[model_name]
    prompt_language = "zh"
    text_language = "zh"
    cut_punc = None
    ...
    if nowLoadModelName != model_name:
        logger.info(
            f"====当前加载的模型为: {nowLoadModelName}, 切换为模型: {model_name}===="
        )
        change_sovits_weights(model_name, device)
        change_gpt_weights(model_name, device)
        nowLoadModelName = model_name

    # Log generation parameters with a timestamp
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    logger.info(
        f"生成语音参数====={refer_wav_path}, {prompt_text}, {prompt_language}, {text}, {text_language}, {model_name} at {current_time}"
    )
...
change_sovits_weights("paimeng", device)
change_gpt_weights("paimeng", device)
...
async def tts_endpoint(request: Request):
    json_post_raw = await request.json()
    return handle(json_post_raw.get("text"), json_post_raw.get("model_name"))
